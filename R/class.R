      ...
      list(
        ...
        add_meta =
          function(dat, dm, d) {
            .self$add_missing_fields(
              dat_1 = dat,
              dm_1 = dm,
              dat_2 = .self$metadata_map$extract(d),
        ...
        decode_and_load_all_datamaps =
          function(con, d) {
            ...
            for (dm in .self$data_maps) {
              ...
              dat = .self$decode_datamap(dm, d)
              # Add any required metadata
              dat = .self$add_meta(dat, dm, d)
        ...
        decode =
          function(con, d, overwrite = T) {
            ...
            if(!overwrite) {
              if(.self$tag_already_loaded(con, d)) {
            ...
            DBI::dbWithTransaction(
              ...
              {
                # Upload tag metadata
                .self$upload_meta(con, d)
                # Upload all other tag data
                .self$decode_and_load_all_datamaps(con, d)
              }
            )
