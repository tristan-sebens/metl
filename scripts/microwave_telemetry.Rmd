...
mt_dm__ = DataMap_MicrowaveTelemetry_XTag_InstantSensorData()
mt_sdm__ = DataMap_MicrowaveTelemetry_XTag_SummarySensorData()

dat = mt_dm__$extract(d)
dat = mt_sdm__$extract(d)

dat_t = mt_sdm__$transform(dat)
...
temp_dat_ = 
  ...
  dplyr::mutate(
    temperature = units::set_units(temperature, '°C')
  )
...
DataMap_MicrowaveTelemetry_XTag_InstantSensorData =
  setRefClass(
    "DataMap_MicrowaveTelemetry_XTag_InstantSensorData",
    contains = "DataMap_MicrowaveTelemetry_XTag_SensorData_Base",
    methods =
      list(
        initialize =
          function(...) {
...
mt_xt_dm$upsert(con, dat_t)

mt_xt_dc = 
  Decoder_MicrowaveTelemetry_XTag(d = mt_template_d)


dat[which(!is.na(dat["Latitude (N)"])),]
...
dat_ = mt_xt_dm$extract(d)

dat_t_ = mt_xt_dm$transform(dat_)  


light_geoloc_dat[["Date"]][[1]]

timechange::time_add(light_geoloc_dat[["Date"]], hour = 23, minute = 59, second = 59)
...
dat = mt_xt_dc$decode_datamap(mt_xt_dc$data_maps[[2]])
