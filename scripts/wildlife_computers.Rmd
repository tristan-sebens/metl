...
dat = 
  wc_mp_inst_dm_$extract(wc_mp_template_d)
...
wc_bs_dm_$input_data_field_map$field_list$START_TIME_FIELD$trans_fn(dat$Start)
```
...
.self = wc_bs_dc_$data_maps[[1]]

dat = .self$extract(d)

.self$transform(dat)

wc_bs_dc_$decode_datamap(wc_bs_dc_$data_maps[[1]])
...
dm_1 = wc_bs_dm_
dat_1 = dm_1$extract(wc_bs_template_d)
dm_2 = wc_mp_inst_dm_
dat_2 = dm_2$extract(wc_mp_template_d)

dm_1$output_data_field_map$common_fields(dm_2$input_data_field_map)
