...
dat__ = 
  lt1000_dm$extract(template_tag_d_ltd_1000_1100_1250)
...
```{r}
d = file.path(lotek_data, "LTD 1300-LTD 1000-LTD 1110-LTD 1250", "Sablefish", "4000")

TagIdentifier()$identify_tag_decoder(d)

dc_ = 
  # Decoder_Lotek.1300(d = d)
  Decoder_Lotek.1000.1100.1250(d = d)
...
```{r}
d = file.path(lotek_data, "LTD 1300-LTD 1000-LTD 1110-LTD 1250", "Sablefish", "3900")
...
# Iterate through every possible pair of the DataMaps, excluding identity pairings
for(i_ in seq(length(t__))) {
  for(j_ in seq(length(t__))) {
    if(i_ == j_) {
      next
    }
    print(paste0(i_, ": ", j_))
...
dat = 
  dc$decode_datamap(dc$data_maps[[1]])

dc$upload_meta(con)

dc$metadata_map$upsert(
  con = con,
  dat = dc$metadata_map$extract(d)
)
...
tag_meta_df = dc$generate_tag_meta_dataframe(TAG_FIELDS)
# Add any meta fields which are common between the FieldMaps of the DataMap and 
# the Decoder
for(f in common_fields) {
  dat[dm$output_data_field_map$field_list[[f]]$name] = 
    dc$generate_tag_meta_dataframe(TAG_FIELDS)[dc$tag_meta_field_map$field_list[[f]]$name]
}
...
dat = dc$decode_datamap(dm = dc$data_maps[[1]])
# Add any required metadata
dat_m = dc$add_meta(dat, dm)
...
dc_1400.1800 = 
  Decoder_Lotek.1400.1800(d = template_tag_fp_lat_1400_d)

dc_1400.1800$decode(con)

fp = list.files(template_tag_fp_lat_1400_d)[[1]]

DataMap_Lotek.1400.1800_TagMetaData()$extract(d = template_tag_fp_lat_1400_d)
DataMap_Lotek.1400.1800_TagMetaData()$get_tag_id(d = template_tag_fp_lat_1400_d)
...
dm_1400.1800 = dc_1400.1800$data_maps[[1]]

dat = dm_1400.1800$extract(template_tag_fp_lat_1400_d)

dm_1400.1800$output_data_field_map
```
...
lt1300_md_dm = DataMap_Lotek.1300_TagMetaData()

lt1300_md_dm$extract(d)

dm = DataMap_Lotek.1300_InstantSensorData()
...
dat_t = dm$transform(dat)

dm$input_data_field_map$field_list$TIMESTAMP_FIELD$trans_fn

length(dat$TimeS)


data.frame(
  dt1 = dat$TimeS[c(1:10, 10000: 10010, 100000:100010)]
) %>% 
  dplyr::mutate(
    dt2 = 
      lubridate::parse_date_time(
        dt1,
        c("%d/%m/%Y %H:%M","%H:%M:%S %d/%m/%y"),
      )
  )
```
